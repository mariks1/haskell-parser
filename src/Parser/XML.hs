...
parseContent parentTag input =
  let (txt, rest) = spanWhile (/= '<') input
      txt' = trim txt
      -- 1) Используем list comprehension
      nodeIfNotEmpty = [NString txt' | not (null txt')]
      ...
      case v of
        NArray items ->
          let normalizedItems = map normalize items
              -- 2) Используем lambda-case для распаковки NObject fs
              mergedPairs =
                concatMap
                  ( \case
                      NObject fs -> fs
...
-- Если объект с несколькими ключами
normalize (NObject fields) =
  -- 3) Используем second
  NObject (map (second normalize) fields)
-- Если массив
normalize (NArray xs) =
...
collectFields (NObject fields) =
  -- 4) Используем second
  map (second normalize) fields
collectFields _ = []
